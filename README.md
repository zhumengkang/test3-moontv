...
- 输出目录：`.vercel/output/static`（Next.js 静态输出与 Edge Functions），或保留默认构建产物（若控制台支持自动识别）。
...
如 EdgeOne Pages 需要专有配置文件（如 `edgeone.json`）来声明重写与缓存策略，请将相应规范文件名与字段告知，我们会补充在仓库根目录并提交。
